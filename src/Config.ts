...
const PACKAGE_MAIN_PROCESS_PATH =
  "@expo/electron-adapter/template/electron/main";
...
function getFolder(
  ...
  const resolvedFullPath =
    resolveFrom.silent(projectRoot, localPath) ||
    resolveFrom.silent(projectRoot, packagePath);
...
export function ensureElectronConfig(projectPath: string) {
  ...
  if (!fs.pathExistsSync(outputPath)) {
    fs.copy(
      path.resolve(__dirname, "../template/electron-webpack.js"),
      outputPath
    );
  }
}

export async function ensureMinProjectSetupAsync(
  projectRoot: string
): Promise<void> {
...
export async function ensureGitIgnoreAsync(projectRoot: string): Promise<void> {
  ...
  console.log(
    chalk.magenta(`\u203A Adding the generated folders to your .gitignore`)
  );
...
function getDependencies(projectRoot: string): {
  ...
  const dependencies = ["react-native-web", "electron@^6.0.12"].filter(
    (dependency) =>
      !resolveFrom.silent(projectRoot, dependency.split("@^").shift()!)
  );
...
async function ensureDependenciesAreInstalledAsync(
  projectRoot: string
): Promise<void> {
  ...
  if (!all.length) {
    console.log(
      chalk.yellow(
        `\u203A All of the required dependencies are installed already.`
      )
    );
    return;
  } else {
    console.log(
      chalk.magenta(
        `\u203A Installing the missing dependencies: ${all.join(", ")}.`
      )
    );
  }
